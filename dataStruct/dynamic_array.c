...
struct _dy_array
{
    ...
    void (* object_free)(object_p object); /*释放方法*/
};
/*---------------------------------------------*\
                创建对象函数
\*---------------------------------------------*/
extern dy_array_p
dy_array_alloc(void) {
    return  dy_array_alloc_init(INIT_CAP);
}
...
dy_array_alloc_init( uint init_cap ) {
    ...
    p->objects = NULL;
    p->objects = Malloc(sizeof(object_p) * init_cap);
...
\*---------------------------------------------*/
extern void
dy_array_remove_at(const dy_array_p p, uint index) {
    uint real_index= 0;
    assert(p != NULL);
